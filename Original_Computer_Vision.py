...
# Threshold jarak untuk menentukan apakah objek botol aqua dan buku dekat dengan pergelangan tangan
threshold = 200
...
book_model = YOLO('yolov8x.pt').to(device) # Model YOLOv8 untuk deteksi objek Book (Buku)
...
def is_book_near_wrist(book_models, keypoints):
    if keypoints is None or len(keypoints) == 0:
        return False
    ...
    for wrist in wrists:  # Loop melalui kedua pergelangan tangan
        ...
        for book_model in book_models:  # Cek jarak setiap sudut kotak bounding box Aqua dengan pergelangan tangan
            x1, y1, x2, y2 = book_model[:4]
            box_corners = [(x1, y1), (x1, y2), (x2, y1), (x2, y2)]
...
while True:
    ...
    if ret:
        ...
        # Deteksi objek Book (Buku) pada frame
        book_results = book_model(frame, device=device)

        # Simpan kotak bounding box dari deteksi Aqua
        aqua_boxes = []
        for r in aqua_results:
            for box in r.boxes:
                if box.cls == 39: # Class Bottle pada coco.yaml
        ...
        # Simpan kotak bounding box dari deteksi Buku
        book_boxes = []
        for r in book_results:
            for box in r.boxes:
                if box.cls == 73: # Class Buku pada coco.yaml
                    book_boxes.append(box.xyxy[0].cpu().numpy())  

                    # Menampilkan Jendela Open CV
                    x1, y1, x2, y2 = map(int, box.xyxy[0].cpu().numpy()[:4])
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 0), 2)  
                    cv2.putText(frame, 'Buku', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)  
        ...
        if aqua_boxes:
            print("Mendeteksi Aqua boxes:", aqua_boxes)
        ...
        # Jika ada deteksi Buku
        if book_boxes:
            print("Mendeteksi Buku boxes:", book_boxes)

        # Cek apakah Buku Aqua dekat dengan pergelangan tangan
        is_near_book_wrist = is_book_near_wrist(book_boxes, keypoints)

        # Tambahkan teks status di frame yang menunjukkan status deteksi
        if is_near_book_wrist and keypoints is not None:
            # Cek ID orang yang mengambil Buku
            taking_book_id = None
            for track in tracks:
                if not track.is_confirmed():
                    continue
                ...
                for book_box in book_boxes:
                    x1, y1, x2, y2 = book_box[:4]
                    if (bbox[0] < x2 and bbox[2] > x1) and (bbox[1] < y2 and bbox[3] > y1):  # Cek overlap
                        taking_book_id = track_id
                        break  # Hentikan jika ditemukan

            # Menampilkan teks pada jendela OpenCV untuk deteksi ambil aqua
            if taking_book_id is not None:
                status_text = f"Orang dengan ID {taking_book_id} mengambil Buku"
                text_color = (0, 255, 0)  # Warna hijau
            else:
                status_text = "Tidak Ada Buku yang diambil"
                text_color = (0, 0, 255)  # Warna merah
        else:
            status_text = "Tidak Ada Buku yang diambil"
            text_color = (0, 0, 255)  # Warna merah

        cv2.putText(frame, status_text, (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, text_color, 2, cv2.LINE_AA)
...
# LINE 160 : 
# Fitur penampilan ini membantu DeepSORT untuk membedakan setiap orang berdasarkan penampilan visual 
# (misalnya, pakaian, warna, bentuk tubuh), yang menambah tingkat akurasi dalam mempertahankan ID saat orang bergerak 

# LINE 176 : 
# Setiap orang yang terdeteksi di frame sebelumnya dan masih terlihat di frame berikutnya akan diperbarui ID-nya oleh DeepSORT. 
# Ini terjadi dengan cara membandingkan bounding box pada frame sebelumnya dengan bounding box yang baru, serta memanfaatkan fitur penampilan.
